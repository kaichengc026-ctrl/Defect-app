...
# 尝试加载原生（native）DLL，优先寻找有导出 C API 的 MvCameraControl.dll
def _load_mvcam_dll():
    candidates = [
        r"C:\\Program Files (x86)\\Common Files\\MVS\\Runtime\\Win64_x64\\MvCameraControl.dll",
        r"C:\\Program Files (x86)\\Common Files\\MVS\\Runtime\\Win64_x64\\MvCameraControl.Net.dll",
        "MvCameraControl.dll",
    ...
    for p in candidates:
        try:
            dll = WinDLL(p)
            # print(f"DEBUG: loaded MvCamera DLL candidate: {p}")
            # 檢查是否匯出我們需要的原生函式
            if hasattr(dll, "MV_CC_EnumDevices"):
                # print(f"DEBUG: selected DLL {p} (exports MV_CC_EnumDevices)")
                return dll
            else:
                # print(f"DEBUG: DLL {p} does not export MV_CC_EnumDevices; trying next candidate")
                last_err = f"{p} loaded but missing MV_CC_EnumDevices"
        except Exception as e:
            last_err = e
    raise OSError(f"Could not load MvCameraControl DLL with required exports (MV_CC_EnumDevices). Last error: {last_err}")

MvCamCtrldll = _load_mvcam_dll()
